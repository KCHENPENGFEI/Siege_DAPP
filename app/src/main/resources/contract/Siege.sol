...
contract testContract {
	function printItem() external returns(uint256);
}
...
contract Siege {
    ...
    */
	...
	function setTestAddr(address addr) public {
		testAddr = addr;
	}

	function getTestAddr() public returns (address) {
		return(testAddr);
	}
  ...
  //   	});
    ...
    */
    ...
    function assetTest() public {
    	SiegeAsset siegeAsset = SiegeAsset(gameAssetAddr);
		// siegeAsset.transfer(msg.sender, rootAddr, 10000, "SIG", "player occupies city");
    }

    function callTest() public returns (uint256) {
    	testContract testC = testContract(testAddr);
    	uint256 data = testC.printItem();
    	return(data);
    }
}
